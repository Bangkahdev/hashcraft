...
strings and verifies SHA-256 digests entirely on the machine it runs
on. It performs no network access, telemetry, remote candidate
submission, credential collection, or online authentication attempts.
Because of that scope, most classes of "remote" vulnerability (data
exfiltration to a third party, remote code execution over a network
protocol, etc.) do not apply to this tool by construction. Security
...
- misrepresent what SHA-256 verification means in a way that could
  mislead a user about a match or non-match (PRD section 7).

Reports about the *inherent* limitations of brute-force/dictionary
candidate generation, or of SHA-256 itself (it is a fast, unsalted
hash -- this is well known and is not a Hashcraft vulnerability), are
out of scope; the README's "Authorized Use Only" section and PRD
section 7's SHA-256 caveat already document these limitations.
